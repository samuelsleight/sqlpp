...
struct MakeResultTuple {
    static auto make(SQLLIB_NS_(DB)* db) {
        auto val = std::tuple_element<N, Tuple>::type::getValue(db, N);
        return std::tuple_cat(MakeResultTuple<Tuple, N - 1>::make(db), std::make_tuple(val));
    }
};

template<typename Tuple>
struct MakeResultTuple<Tuple, 0> {
    static auto make(SQLLIB_NS_(DB)* db) {
        auto val = std::tuple_element<0, Tuple>::type::getValue(db, 0);
